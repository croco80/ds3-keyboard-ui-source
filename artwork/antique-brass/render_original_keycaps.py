...
def draw_mouse(draw: ImageDraw.ImageDraw, *, left: float, top: float, width: float, height: float, button: str) -> None:
    ...
    if button in {"left", "right"}:
        ...
        draw.rectangle((bx0, y0 + u(1.25), bx1, split_y), fill=(185, 139, 70, 245))
...
def shift_mouse_symbol(button: str) -> Image.Image:
    ...
    draw.line(points([(14.25, 15.8), (15.75, 15.8)]), fill=BRASS_LIGHT, width=u(0.55))
    draw.line(points([(15.0, 15.05), (15.0, 16.55)]), fill=BRASS_LIGHT, width=u(0.55))
    return symbol
...
def build_preview(rendered: dict[str, Image.Image]) -> None:
    ...
    draw.text((640, 42), "ORIGINAL KEYBOARD & MOUSE PROMPTS", font=title_font, fill=(235, 224, 198), anchor="mm")
    draw.text(
        (640, 82),
        "Clean-room artwork • antique iron, aged brass, and consistent typography",
        font=subtitle_font,
        fill=(164, 150, 122),
        anchor="mm",
    )
    for index, name in enumerate(ordered):
        ...
        draw.rounded_rectangle(cell, radius=14, fill=(21, 21, 25), outline=(65, 53, 36), width=2)
    ...
    canvas.save(PREVIEW_DIR / "DS3_Original_Keycaps_Preview.png", optimize=True)
    ...
    qa.save(PREVIEW_DIR / "DS3_Original_Keycaps_32px_QA.png", optimize=True)
...
def main() -> None:
    ...
    print(f"Rendered {len(rendered)} target textures.")
    print(PREVIEW_DIR / "DS3_Original_Keycaps_Preview.png")
